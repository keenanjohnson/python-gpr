...
def test_pep518_build_system():
    ...
    ), "build-system.requires should be a list"
    assert (
        len(build_system["requires"]) > 0
    ), "build-system.requires should not be empty"

    assert (
        build_system["build-backend"] == "scikit_build_core.build"
    ), "Expected scikit-build-core backend"
...
def test_pep621_project_metadata():
    ...
    assert ("version" in project) or (
        "dynamic" in project and "version" in project["dynamic"]
...
def test_project_name():
    ...
    assert (
        data["project"]["name"] == "python-gpr"
    ), "Project name should be 'python-gpr'"
...
def test_optional_dependencies():
    ...
    ), "optional-dependencies should be specified"
    ...
    if "urls" in project:
        ...
        for url_type in expected_urls:
            if url_type in urls:
                ...
                assert urls[url_type].startswith(
                    "http"
                ), f"{url_type} URL should be a valid URL"
